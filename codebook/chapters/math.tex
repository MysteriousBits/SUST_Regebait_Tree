...
\begin{center}
\includegraphics[width=25mm]{graphics/sphericalCoordinates}
\end{center}
\[
\begin{aligned}
  x &= r\sin\theta\cos\phi & r &= \sqrt{x^2+y^2+z^2}\\
  y &= r\sin\theta\sin\phi & \theta &= \arccos(z/r)\\
  z &= r\cos\theta & \phi &= \operatorname{atan2}(y,x)
\end{aligned}
\]
...
\section{Sums}
% Geometric Sum (shorthand)
\[ \sum_{i=a}^{b} c^i = \frac{c^{b+1} - c^a}{c-1}, c \neq 1 \]

\begin{align*}
  \textstyle\sum i &= \tfrac{n(n+1)}{2} \\
  \textstyle\sum i^2 &= \tfrac{n(n+1)(2n+1)}{6} \\
  \textstyle\sum i^3 &= \tfrac{n^2(n+1)^2}{4} \\
  \textstyle\sum i^4 &= \tfrac{n(n+1)(2n+1)}{30} (3n^2 + 3n - 1)
\end{align*}

\section{Series}
{\small
\begin{align*}
  e^x &= \textstyle\sum_{n=0}^{\infty} \frac{x^n}{n!} \\
  &\text{\scriptsize all } x \\
  \ln(1+x) &= x - \tfrac{x^2}{2} + \tfrac{x^3}{3} - \dots \\
  &\text{\scriptsize } (-1 < x \le 1) \\
...
\raggedright
The number of successes in $n$ independent yes/no experiments, each yielding success with probability $p$, is $\textrm{Bin}(n,p)$.
...
\raggedright
If PDF is constant on $[a, b]$ and 0 elsewhere, it is $\textrm{U}(a,b),\,a<b$.
\[f(x) = \left\{
\begin{array}{cl}
\frac{1}{b-a} & a<x<b\\
0 & \textrm{otherwise}
...
\subsection{Euler's Totient ($\phi$)}
\raggedright
Counts integers $1 \le k \le n$ coprime to $n$.
...
\[
\mu(n) = \begin{cases} 
  0 & \text{\small $n$ not square-free} \\ 
  1 & \text{\small even \# prime factors} \\ 
  -1 & \text{\small odd \# prime factors}
\end{cases}
\]
