...
\fancyhead[R]{\thepage}
\fancyhead[L]{Shahjalal University of Science and Technology - FourierDominion}
\setlist[enumerate]{leftmargin=17pt, itemsep=2pt, parsep=0pt}
\begin{document}
\title{\vspace{-1.5cm}FourierDominion. \\ \large \textit{sys.set\_int\_max\_str\_digits(10000); from heapq import *; heap = []; heappush(heap, ...); heapop(heap); } \\ \large \textit{list(map(int, input().strip().split()))} }
...
struct Node {
 int p = 0, c[2] = {0, 0}, pp = 0, sz = 0, ssz = 0, vsz = 0, mn_id = 0;
 bool flip = 0;
 long long val = INF, sum = 0, lazy = 0, subsum = 0, vsum = 0, mn = INF;
 Node() {}
 Node(int id, long long x) {
  val = sum = subsum = mn = x; mn_id = id;
  sz = ssz = 1; lazy = vsz = vsum = 0;
 }
};
...
struct LCT {
 vector<Node> t;
 LCT() {}
 LCT(int n, int m = 0) : t(n + m + 1) {
  for (int i = 0; i <= n + m; i++) t[i] = Node(i, INF);
 }
 ...
 void pull(int x) {
  if (!x) return;
  int &l = t[x].c[0], &r = t[x].c[1]; push(l); push(r);
  ...
  t[x].mn = t[x].val; t[x].mn_id = x;
  if (l && t[l].mn < t[x].mn) t[x].mn = t[l].mn, t[x].mn_id = t[l].mn_id;
  if (r && t[r].mn < t[x].mn) t[x].mn = t[r].mn, t[x].mn_id = t[r].mn_id;
 }
 void push(int x) {
  if (!x) return;
  ...
  if (t[x].flip) {
   swap(l, r);
   if (l) t[l].flip ^= 1; if (r) t[r].flip ^= 1;
   t[x].flip = 0;
  }
  if (t[x].lazy) {
   t[x].val += t[x].lazy; t[x].mn += t[x].lazy;
   t[x].sum += t[x].lazy * t[x].sz;
   t[x].subsum += t[x].lazy * t[x].ssz;
   t[x].vsum += t[x].lazy * t[x].vsz;
   if (l) t[l].lazy += t[x].lazy; if (r) t[r].lazy += t[x].lazy;
   t[x].lazy = 0;
  }
 }
 ...
 void link(int u, int v) {
  ...
  t[v].pp = u; t[u].vsz += t[v].ssz; t[u].vsum += t[v].subsum;
 }
 void cut(int u, int v) {
  make_root(u); access(v); splay(v);
  if (t[v].c[0] == u && !t[u].c[1]) {
   t[v].c[0] = t[u].p = 0; pull(v);
  }
 }
 void link_edge(int e, int u, int v) { link(u, e); link(e, v); }
 void cut_edge(int e, int u, int v) { cut(u, e); cut(e, v); }
 void set_val(int u, long long x) { access(u); splay(u); t[u].val = x; pull(u); }
 pair<long long, int> query(int u, int v) {
  make_root(u); access(v); splay(v); return {t[v].mn, t[v].mn_id};
 }
 int find_root(int u) {
...
% \subsection{LR Flow (Circulation with Demands)}
% \textbf{Problem:} Edges have lower bounds $L$ and upper bounds $R$. 

% \textbf{Setup:}
% \begin{enumerate}
%     \item Create a Super-Source ($SS$) and Super-Sink ($TT$).
%     \item Maintain a demand array $D$ initialized to $0$.
%     \item For each original edge $u \to v$ with bounds $[L, R]$:
%     \begin{itemize}
%         \item Add a standard flow edge $u \to v$ with capacity $R - L$.
%         \item $D[v] \mathrel{+}= L$ (incoming requirement).
%         \item $D[u] \mathrel{-}= L$ (outgoing requirement).
%     \end{itemize}
%     \item For each node $i$:
%     \begin{itemize}
%         \item If $D[i] > 0$: Add edge $SS \to i$ with capacity $D[i]$.
%         \item If $D[i] < 0$: Add edge $i \to TT$ with capacity $-D[i]$.
%     \end{itemize}
%     \item \textbf{Feasibility Check:} Run Dinic's from $SS$ to $TT$. A valid circulation exists \emph{if and only if} Max Flow equals the sum of all positive $D[i]$.
%     \item \textbf{Max Flow with Bounds (Source $S$ to Sink $T$):} Add an edge $T \to S$ with capacity $\infty$. Run feasibility from $SS$ to $TT$. The base flow is the flow pushed through $T \to S$. Remove $T \to S$, then run Dinic's from $S$ to $T$ on the residual graph to maximize it.
% \end{enumerate}
...
% \subsection{Maximum Independent Set (MIS) - Bipartite}
% \textbf{Formula:} $|MIS| = V - |MBM|$

% \textbf{Setup:}
% \begin{enumerate}
%     \item Build a standard bipartite matching graph: $S \to \text{Left}$ (cap 1), $\text{Left} \to \text{Right}$ (cap 1), $\text{Right} \to T$ (cap 1).
%     \item Run Dinic's from $S$ to $T$ to find the MBM.
% \end{enumerate}

% \textbf{Extraction (Finding the specific vertices):}
% \begin{enumerate}
%     \item Run a simple BFS/DFS starting from $S$ strictly on the \textbf{residual graph} (only traverse edges where $capacity - flow > 0$).
%     \item Mark all visited vertices.
%     \item The Maximum Independent Set consists of:
%     \begin{itemize}
%         \item \textbf{Left} nodes that are \textbf{VISITED}.
%         \item \textbf{Right} nodes that are \textbf{UNVISITED}.
%     \end{itemize}
% \end{enumerate}
...
% \subsection{Min Cost Vertex Cover - Bipartite}
% \textbf{Problem:} Pick a set of vertices to cover all edges, minimizing the sum of the picked vertices' weights.

% \textbf{Setup (Max-Flow Min-Cut):}
% \begin{enumerate}
%     \item For each Left node $u$: Add $S \to u$ with capacity $W_u$.
%     \item For each Right node $v$: Add $v \to T$ with capacity $W_v$.
%     \item For each bipartite edge $(u, v)$: Add $u \to v$ with capacity $\infty$.
%     \item Run Dinic's from $S$ to $T$. The Min Cost Vertex Cover is exactly equal to the Max Flow.
% \end{enumerate}
...
%     \item Run a simple BFS/DFS starting from $S$ strictly on the \textbf{residual graph} (only traverse edges where $capacity - flow > 0$).
%     \item Mark all visited vertices.
%     \item The Min Cost Vertex Cover consists of:
%     \begin{itemize}
%         \item \textbf{Left} nodes that are \textbf{UNVISITED}.
%         \item \textbf{Right} nodes that are \textbf{VISITED}.
%     \end{itemize}
% \end{enumerate}
...
\subsection{Some Flow Problems}
{\footnotesize % Options: \small, \footnotesize, \scriptsize, \tiny
\subsection*{1. Bipartite Matching (Basic \& Global Resource)}
\textbf{Brief:} Assign items from Set L to Set R. Variations include workers having maximum job capacities, or a "global potion" that allows a limited number of extra assignments overall.

\textbf{Construction:}
\begin{enumerate}
    \item Connect $S \to L$ (capacity 1, or $C_i$ if a worker can take multiple jobs).
    \item Connect $L \to R$ (capacity 1).
    \item Connect $R \to T$ (capacity 1).
    \item \textbf{Joker/Potion Trick:} If a global resource allows $K$ extra connections anywhere, add a special node. Connect $S \to \text{Joker}$ (capacity $K$). Connect $\text{Joker} \to L$ (capacity 1).
    \item Run Dinic's. Max flow is the maximum valid pairs.
\end{enumerate}


\subsection*{2. Min Vertex Cover \& Max Independent Set (Grid/Bipartite)}
\textbf{Brief:} Find the minimum vertices to cover all edges, or the maximum vertices such that no two are connected. Often framed on 2D grids (e.g., destroying targets on rows/columns).

\textbf{Construction:}
\begin{enumerate}
    \item \textbf{Grid Parsing:} Left nodes = Rows (or contiguous horizontal segments). Right nodes = Columns (or vertical segments). Add Left $\to$ Right (capacity 1) if a target exists.
    \item Connect $S \to$ Left (cap 1), Right $\to T$ (cap 1). Run Dinic's to find Max Bipartite Matching (MBM).
    \item \textbf{Math:} $|MIS| = V - |MBM|$.
    \item \textbf{Extraction:} Run BFS from $S$ strictly on the \emph{residual graph} (where $cap - flow > 0$). 
    \item Min Cover = (Unvisited Left) $\cup$ (Visited Right).
    \item MIS = (Visited Left) $\cup$ (Unvisited Right).
\end{enumerate}


\subsection*{3. Project Selection (Max Weight Closure)}
\textbf{Brief:} Choose a subset of items to maximize profit. Some yield profit, some cost money. Picking item A strictly requires picking item B.

\textbf{Construction:}
\begin{enumerate}
    \item Connect $S \to i$ with capacity $=$ Profit (for items with positive profit).
    \item Connect $i \to T$ with capacity $= |\text{Profit}|$ (for items with negative profit).
    \item For a dependency "A requires B", add a directed edge $A \to B$ with capacity $\infty$.
    \item Run Dinic's. Total Profit $=$ (Sum of all positive profits) $-$ (Min-Cut).
\end{enumerate}


\subsection*{4. Image Segmentation (2-Partitioning with Penalties)}
\textbf{Brief:} Assign each element to either Set A or Set B to maximize score. Gain $A_i$ for Set A, $B_i$ for Set B. Incur penalty $P_{ij}$ if adjacent elements $i$ and $j$ are put in different sets.

\textbf{Construction:}
\begin{enumerate}
    \item Connect $S \to i$ with capacity $A_i$ and $i \to T$ with capacity $B_i$.
    \item For penalty pairs, add a bidirectional edge $i \leftrightarrow j$ with capacity $P_{ij}$.
    \item \textbf{Forced Alignment:} If $u \in A \implies v \in A$, add directed edge $u \to v$ with capacity $\infty$.
    \item Run Dinic's. Answer $=$ (Sum of all $A_i$ and $B_i$) $-$ (Min-Cut).
\end{enumerate}


\subsection*{5. Node Capacities \& Node Severing}
\textbf{Brief:} The graph has costs or capacities on the \emph{vertices} instead of the edges, or the problem asks for the minimum cost to disconnect $S$ and $T$ by destroying vertices.

\textbf{Construction (Vertex Splitting):}
\begin{enumerate}
    \item Split every node $u$ into two nodes: $u_{in}$ and $u_{out}$.
    \item Add a directed edge $u_{in} \to u_{out}$ with capacity $=$ node's limit or destruction cost.
    \item For every original directed edge $u \to v$, add a directed edge $u_{out} \to v_{in}$ with capacity $\infty$.
    \item Run Dinic's. The Min-Cut will sever the internal $u_{in} \to u_{out}$ edges.
\end{enumerate}


\subsection*{6. Binary Search + Max Flow (Minimax)}
\textbf{Brief:} "Minimize the maximum load", "find the maximum capacity threshold", or "find the minimum range to cover all points."

\textbf{Construction:}
\begin{enumerate}
    \item Binary search the answer $X$ (the threshold/time/load limit).
    \item Inside the \texttt{check(X)} function, build a fresh flow network. Only add edges if their cost/distance is $\le X$, or cap their capacities at $X$.
    \item Run Dinic's. If Max Flow $\ge$ required target (e.g., all demands met), $X$ is valid. Update binary search bounds.
\end{enumerate}


\subsection*{7. Tournament Elimination (Game Feasibility)}
\textbf{Brief:} Given current standings and remaining match schedules, can Team X theoretically win or tie the championship?

\textbf{Construction:}
\begin{enumerate}
    \item Assume Team X wins all remaining games to reach final score $W_X$. If any team $i$ already has $> W_X$ points, X is eliminated.
    \item Team $i$ can win at most $W_X - \text{current\_score}_i$ more games. Connect $i \to T$ with this capacity.
    \item For every remaining match between team A and B, create a Match Node. Connect $S \to \text{Match}$ (cap $=$ games left between A and B).
    \item Connect $\text{Match} \to A$ (cap $\infty$) and $\text{Match} \to B$ (cap $\infty$). Team X can win if Max Flow $=$ total remaining matches.
\end{enumerate}


\subsection*{8. Flow / Cut strictly over Shortest Paths}
\textbf{Brief:} Push flow or find a bottleneck (Min-Cut) from $S$ to $T$, but flow is strictly restricted to edges that lie on an absolute shortest path from $S$ to $T$.

\textbf{Construction:}
\begin{enumerate}
    \item Run Dijkstra/BFS from $S$ to get array \texttt{distS}.
    \item Run Dijkstra/BFS from $T$ on the \emph{reversed} graph to get array \texttt{distT}.
    \item Let $D = \texttt{distS}[T]$. Build a new empty flow network.
    \item For every original edge $u \to v$ with weight $W$: add it to the flow network if and only if $\texttt{distS}[u] + W + \texttt{distT}[v] == D$.
    \item Run Dinic's on this filtered graph (which is guaranteed to be a DAG).
\end{enumerate}


\subsection*{9. Minimum Path Cover \& Dilworth's Theorem}
\textbf{Brief:} Find the minimum number of paths to cover all vertices in a DAG, or find the largest set of mutually unreachable vertices (Antichain).

\textbf{Construction:}
\begin{enumerate}
    \item Split every node $u \to (u_{in}, u_{out})$. 
    \item For every original directed edge $u \to v$, add $u_{out} \to v_{in}$ (cap 1). 
    \item Connect $S \to u_{out}$ (cap 1) and $v_{in} \to T$ (cap 1). 
    \item Min Path Cover $= V - \text{Max Flow}$.
    \item \emph{Dilworth's Theorem:} Largest Antichain $=$ Min Path Cover of the \emph{Transitive Closure} (run Floyd-Warshall/BFS first to add direct edges between all reachable pairs).
\end{enumerate}


\subsection*{10. Circulation with Demands (LR Flow)}
\textbf{Brief:} Find a valid flow or construct a matrix where every edge/cell has a strict lower bound $L$ and upper bound $R$, maintaining node balance.

\textbf{Construction:}
\begin{enumerate}
    \item For every edge $u \to v$ with bounds $[L, R]$, add flow edge $u \to v$ with capacity $R - L$.
    \item Maintain array $D[v] = \text{Incoming } L - \text{Outgoing } L$.
    \item Add Super-Source ($SS$) and Super-Sink ($TT$). 
    \item If $D[v] > 0$, add $SS \to v$ (cap $D[v]$). If $D[v] < 0$, add $v \to TT$ (cap $-D[v]$).
    \item Run Dinic's from $SS$ to $TT$. Valid if Max Flow equals sum of all positive $D[v]$. Actual flow on an edge is $L + \text{flow\_pushed}$.
\end{enumerate}


\subsection*{11. Time-Expanded Networks (Dynamic Flow)}
\textbf{Brief:} Entities move across a graph where each traversal takes 1 unit of time. Find if a certain amount of flow can reach the destination within $K$ time steps.

\textbf{Construction:}
\begin{enumerate}
    \item Create a distinct copy of every node for every time step $t \in [0, K]$. Node $u$ becomes $u_0, u_1, \dots u_K$.
    \item Add "waiting" edges: $u_t \to u_{t+1}$ with capacity $\infty$.
    \item For every real edge $u \to v$ with capacity $C$, add a crossing edge $u_t \to v_{t+1}$ with capacity $C$.
    \item Connect $S$ to start nodes at $t=0$. Connect destination nodes at all $t$ to $T$. Run Dinic's.
\end{enumerate}


\subsection*{12. MCMF: Interval Scheduling \& K-Routing}
\textbf{Brief:} Route items or schedule intervals where each action has a cost, minimizing total cost (e.g., $K$-disjoint paths with cheapest sum, or interval scheduling with $K$ workers).

\textbf{Construction (Interval Scheduling):}
\begin{enumerate}
    \item Coordinate-compress all start/end times into $V$ points. 
    \item Add timeline edges $t \to t+1$ with capacity $K$ (workers), cost $0$.
    \item Add job edges $\text{start} \to \text{end}$ with capacity 1, cost $= -\text{profit}$.
    \item Connect $S \to 1$ (cap $K$, cost $0$) and $V \to T$ (cap $K$, cost $0$). Run MCMF. Total cost is negative; invert for max profit.
\end{enumerate}


% \subsection*{13. Planar Graph Min-Cut (Dual Graph Trick)}
% \textbf{Brief:} Find the Min-Cut in a massive 2D planar graph (e.g., $1000 \times 1000$ grid) where $\mathcal{O}(V^2 E)$ flow will Time Limit Exceed.

% \textbf{Construction:}
% \begin{enumerate}
%     \item The Min-Cut of a planar graph is strictly equal to the Shortest Path in its Dual Graph.
%     \item Treat every enclosed polygon/face as a node. 
%     \item If an original edge (weight $W$) separates Face A and Face B, add an undirected edge between Dual Node A and Dual Node B with weight $W$.
%     \item Run Dijkstra from the exterior face representing $S$ to the exterior face representing $T$ in $\mathcal{O}(E \log V)$.
% \end{enumerate}


% \subsection*{14. All-Pairs Min-Cut (Gomory-Hu Tree)}
% \textbf{Brief:} You are given an undirected graph and $Q$ queries asking for the Min-Cut between arbitrary nodes $u$ and $v$.

% \textbf{Implementation (Gusfield's Algorithm using Dinic's):}
% \begin{enumerate}
%     \item A Gomory-Hu tree compresses all pairwise Min-Cuts into a single spanning tree.
%     \item Create a \texttt{parent} array $P$ of size $V$, initialized to $1$ (all nodes parented to node 1).
%     \item Loop $i$ from $2$ to $V$:
%     \begin{itemize}
%         \item Rebuild the original unmodified graph. Run Dinic's with $S = i$ and $T = P[i]$.
%         \item Add an undirected edge in your Gomory-Hu Tree between $i$ and $P[i]$ with weight $=$ Max Flow.
%         \item Run a simple BFS/DFS starting from $i$ on the \emph{residual graph} (where $cap - flow > 0$). Mark all visited nodes.
%         \item Loop $j$ from $i+1$ to $V$: If $P[j] == P[i]$ and $j$ was \emph{visited} in the BFS, update $P[j] = i$.
%     \end{itemize}
%     \item You now have a tree with $V-1$ edges. To answer a query for the Min-Cut between $U$ and $V$, find the path between them on this tree. The answer is the \emph{minimum edge weight} on that exact path.
% \end{enumerate}
} % Ends the font size scope
...
\begin{minted}
void FMT(vector<int>& a, bool inv) {
  int n = (int)a.size() - 1;
  vector<int> primes;
  ...
  for (int p : primes) {
    if (!inv) for (int i = n / p; i >= 1; i--) a[i] += a[i * p]; // GCD
    else      for (int i = 1; i <= n / p; i++) a[i] -= a[i * p];
    // if (!inv) for (int i = 1; i <= n / p; i++) a[i * p] += a[i]; /// LCM
    // else      for (int i = n / p; i >= 1; i--) a[i * p] -= a[i]; /// LCM
  }
}
vector<int> conv(vector<int> a, vector<int> b) {
  FMT(a, 0); FMT(b, 0);
...
\end{minted}
\subsection{Determinant and Inverse Matrix}
\begin{minted}{c++}
bool Inv(vector<vector<int>> a, vector<vector<int>> &ans) {
  int n = a.size();
  vector<int> col(n);
...
int Det(vector<vector<int>> a) {
  ...
  for (int col = 0, row = 0; col < n && row < n; col++) {
    ...
    for (int j = col; j < n; j++) swap(a[mx][j], a[row][j]);
    ...
    for (int i = 0; i < n && inv; i++) {
      if (i != row && a[i][col]) {
        ...
        for (int j = col; j < n && x; j++) {
          if (a[row][j]) a[i][j] = (MODSQ + a[i][j] - 1LL * a[row][j] * x) % mod;
        }
      }
    }
    row++;
...
\begin{minted}{c++}
int BASES[] = {31, 37, 41, 43, 47, 53, 59, 61, 67, 71, 137, 139, 149, 151, 157, 257, 263, 269, 271, 277, 311, 313, 317, 331, 347};
911382, 972663, 1e9+7, 1e9+9, 1e9+21, 1e9+33, 1e9+87, 1e9+93, 1e9+97
1e18+21, 1e18+33, 1e18+87, 1e18+93, 2e18+11, 2e18+33, 2e18+63, 2e18+87, 4611686018427387847, 9223372036854775783
pair<int, int> ntt_primes = {{(119 << 23) + 1, 3}, {(5 << 25) + 1, 3}, {(7 << 26) + 1, 3},{(479 << 21) + 1, 3}, {(483 << 21) + 5, 5}};
\end{minted}
\subsection{mobius}
\begin{minted}{c++}
// returns mu[x]  O(logn)
